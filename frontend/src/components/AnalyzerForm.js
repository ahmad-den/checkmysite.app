        ...
        <label className="flex items-center cursor-pointer">
          <div className="relative">
            <input
              ...
              className="sr-only"
            />
            <div className={`block w-12 h-7 rounded-full transition duration-300 ease-in-out ${coreWebVitals ? 'bg-[#32CC86] shadow-inner' : 'bg-gray-300 shadow-inner'}`}></div>
            <div className={`absolute top-1 left-1 w-5 h-5 rounded-full bg-white transition-transform duration-300 ease-in-out transform ${coreWebVitals ? 'translate-x-5' : 'translate-x-0'} shadow-lg`}>
              {/* Adding gradient for the knob */}
              <div className="absolute inset-0 rounded-full bg-gradient-to-tl from-white to-gray-200 shadow-md"></div>
            </div>
          </div>
          <span className="ml-3 text-gray-900">Run Core Web Vitals (via Lighthouse)</span>
        </label>
      </div>
